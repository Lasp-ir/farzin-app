...
export default function OpeningExplorer({ fen, onMoveSelect }: OpeningExplorerProps) {
  ...
  const saveToken = () => {
    ...
    setIsConnectModalOpen(false); // بستن مودال بعد از ثبت موفق
  }
  ...
  // 🌟 مودال (پاپ‌آپ) مدرن برای دریافت توکن
      ...
      return (
          <>
            <div className="flex flex-col items-center justify-center h-full text-zinc-400 p-5 gap-4 text-center bg-[#161512] rounded-xl border border-[#35332e]">
                <div className="w-16 h-16 bg-purple-500/10 text-purple-400 flex items-center justify-center rounded-2xl shadow-[0_0_20px_rgba(168,85,247,0.2)] border border-purple-500/20">
                    <Database size={32} />
                </div>
                <div className="flex flex-col gap-1.5">
                    <h3 className="font-black text-white text-sm">دیتابیس گشایش‌ها</h3>
                    <p className="text-[11px] leading-relaxed opacity-80 px-2 text-zinc-400">
                        برای مشاهده‌ی آمار دقیق حرکات بازیکنان، باید مستقیماً به سرورهای لیچس متصل شوید.
                    </p>
                </div>
                <button 
                    onClick={() => setIsConnectModalOpen(true)} 
                    className="mt-3 bg-purple-500 hover:bg-purple-400 text-white px-5 py-3 rounded-xl font-bold text-xs transition-all shadow-[0_5px_15px_rgba(168,85,247,0.3)] active:scale-95 flex items-center gap-2"
                >
  ...
  return (
    <div className="flex flex-col h-full bg-[#161512] rounded-xl border border-[#35332e] overflow-hidden relative" dir="ltr">
      <button onClick={() => setIsConnectModalOpen(true)} className="absolute top-2 right-2 p-1.5 text-zinc-500 hover:text-white bg-[#262421] border border-[#35332e] rounded-lg opacity-0 hover:opacity-100 transition-opacity z-10" title="تغییر کلید ارتباطی">
          <KeyRound size={12} />
      </button>
